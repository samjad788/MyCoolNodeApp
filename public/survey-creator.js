...
function init() {
    ...
    $.ajax({
        ...
        success: function(theme) {
            ...
            surveyCreator.isAutoSave = false;
            surveyCreator.showState = true;
            surveyCreator.showOptions = false;

            setSurveyName(surveyName);

            surveyCreator.onModified.add(function(result, options) {
                updateTheme(theme)
                setTimeout(updateTheme, 500, theme);
            });

            surveyCreator.onTestSurveyCreated.add(function(result, options) {
                updateTestTheme(theme)
                setTimeout(updateTestTheme, 500, theme);
            });
        }
    });
}
...
function updateTheme(theme) {
    ...
    $("h5.title_editable").css("color", "#" + theme.q_textcolor);
    $("h5.title_editable").css("background", "#" + theme.q_backcolor);

    if (!loadFont) addFontLink(theme.q_font);
    var q_font = parseFont(theme.q_font);
    $('h5.title_editable').css({ fontFamily: "'" + q_font[0] + "'", fontWeight: q_font[1] });

    $("fieldset, fieldset table, table.table, div.btn-group label.btn, .sv-boolean").css("color", "#" + theme.a_textcolor);

    if (!loadFont) addFontLink(theme.a_font);
    loadFont = true;
    var a_font = parseFont(theme.a_font)
    $("fieldset, fieldset table, table.table, div.btn-group label.btn, .sv-boolean").css({ fontFamily: "'" + a_font[0] + "'", fontWeight: a_font[1] });

}

function updateTestTheme(theme) {
    $("div h5").css("color", "#" + theme.q_textcolor);
    $("div h5").css("background", "#" + theme.q_backcolor);

    if (!loadFont) addFontLink(theme.q_font);
    var q_font = parseFont(theme.q_font);
    $('div h5').css({ fontFamily: "'" + q_font[0] + "'", fontWeight: q_font[1] });
...
function addFontLink(font) {
    if (googleFonts.indexOf(font) > -1) {
        $('link:last').after('<link href="' + "https://fonts.googleapis.com/css?family=" + font + '" rel="stylesheet" type="text/css">');
    }
}
